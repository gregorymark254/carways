...
const Admin = () => {
  ...
  const getBooking = async () => {
    const response = await axios.get("http://localhost4000/api/v3/all");
        ...
        <table className="w-full text-sm text-left mt-10">
          ...
          <tbody>
            {booking.map((booking, index) => (
              <tr key={booking._id} className="bg-white border-b border-slate-500">
                ...
                {/* <td className="flex items-center text-xl py-4 px-6 space-x-2">
                  <Link to={`/edit/${booking._id}`} className="bg-teal-500 p-1 text-md text-white mr-1"><FaEdit/></Link>
                  <button onClick={() => deletebooking(booking._id)} className="bg-red-500 p-1 text-md text-white"><MdDelete/></button>
                </td> */}
              </tr>
            ))}
          </tbody>
